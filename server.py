...
/* Wake-up banner */
.wake-banner{background:var(--amber-dim);border:1px solid rgba(240,168,48,.3);border-radius:10px;padding:1rem 1.5rem;margin-bottom:1.5rem;display:flex;align-items:center;gap:12px;font-size:13px;color:var(--amber)}
...
<div id="loading">
  ...
  <div style="font-size:11px;color:var(--muted);margin-top:8px" id="wake-hint"></div>
</div>
<header>
  <div class="logo">
...
<main>
  <div id="wake-banner" class="wake-banner hidden">
    ⏳ Servidor acordando do modo de espera — isso pode levar até 60 segundos na primeira vez. Aguarde...
  </div>
  <div class="tabs">
...
// Keep-alive: ping o servidor a cada 10 minutos para evitar hibernacao
setInterval(()=>{fetch('/ping').catch(()=>{});},10*60*1000);

async function fetchWithRetry(url, timeout=180000){
  // Tenta ate 2 vezes com timeout de 3 minutos (para pegar o warm-up do Render)
  for(let attempt=0;attempt<2;attempt++){
    ...
    try{
      ...
      clearTimeout(tid);
      if(attempt===0 && (e.name==='AbortError'||e.message.includes('fetch'))){
        // primeira tentativa falhou — provavelmente warm-up; avisa e tenta de novo
        document.getElementById('lt').textContent='Servidor acordando, aguarde...';
        document.getElementById('wake-hint').textContent='O Render pode levar ate 60s para iniciar apos inatividade.';
...
async function loadAll(){
  ...
  document.getElementById('wake-hint').textContent='';
  document.getElementById('wake-banner').classList.add('hidden');
  setLoad(10,'Buscando dados...');
  try{
    setLoad(20,'Buscando Funil RP...');
    const rpRes=await fetchWithRetry(`/api/data?funil=rp&month=${selM}&year=${selY}`);
    const rp=await rpRes.json();

    setLoad(55,'Buscando Funil RRR Mae...');
    const rrrRes=await fetchWithRetry(`/api/data?funil=rrr&month=${selM}&year=${selY}`);
    ...
    document.getElementById('loading').style.display='none';
    const pane=document.getElementById('pane-'+curF);
    pane.innerHTML=`<div style="background:var(--red-dim);border:1px solid rgba(240,96,96,.3);border-radius:10px;padding:1.25rem;color:var(--red);font-size:13px"><strong>Erro ao carregar:</strong> ${e.message}<br><br><span style="color:var(--dim);font-size:12px">Tente clicar em "Atualizar". Se o servidor estiver acordando, aguarde 30-60 segundos.</span></div>`;
...
function renderPane(key){
  ...
  h+=renderD1(s.contrato_d1,null,s.contrato_hoje,null,key);
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/":
            ...
            # Keep-alive endpoint para evitar hibernacao do Render
            body = b'{"ok":true}'
            self.send_response(200)
