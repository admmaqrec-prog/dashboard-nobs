...
h+=`<div class="resp-card"> <div class="resp-header"> <div class="resp-avatar" style="background:${color}22;color:${color}">${init}</div> <div style="flex:1;min-width:0"><div style="display:flex;align-items:center;gap:8px"><div class="resp-name">${name}</div><span style="font-family:'DM Mono',monospace;font-size:13px;font-weight:700;color:var(--blue)">${data.contratos.length}c</span></div> <div style="font-size:11px;color:var(--muted);font-family:'DM Mono',monospace">${data.contratos.length} contratos · ${data.vendas.length} vendas no mes</div></div> <div class="resp-total" style="color:${color}">${data.contratos.length}</div> </div><div class="resp-rows">`;
...
h+=`<div class="resp-row"><span class="resp-row-label"><span class="resp-row-dot" style="background:var(--red)"></span>Perdas</span><span class="resp-row-val" style="color:var(--red)">${data.perdas}</span></div> <div class="resp-row"><span class="resp-row-label"><span class="resp-row-dot" style="background:var(--teal)"></span>Valor vendas no mes</span><span class="resp-row-val" style="color:var(--teal);font-size:12px">${fmoney(data.valor)}</span></div> </div></div>`;
});
h+=`</div>`;
...
let h=`
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:2rem">
    <div class="summary-card purple"><div class="sc-label">Vendas por midia social</div><div class="sc-val purple">${totBuscaPagaV}</div><div class="sc-sub">ambos funis · origem "busca"</div></div>
...
const uid=`tot-${i}`;
h+=`<div class="resp-card"> <div class="resp-header"> <div class="resp-avatar" style="background:${color}22;color:${color}">${init}</div> <div style="flex:1;min-width:0"><div style="display:flex;align-items:center;gap:8px"><div class="resp-name">${name}</div><span style="font-family:'DM Mono',monospace;font-size:13px;font-weight:700;color:var(--blue)">${data.contratos.length}c</span></div> <div style="font-size:11px;color:var(--muted);font-family:'DM Mono',monospace">${data.contratos.length} contratos · ${data.vendas.length} vendas no mes</div></div> <div class="resp-total" style="color:${color}">${data.contratos.length}</div> </div><div class="resp-rows">`;
h+=`<div class="resp-row" style="cursor:${data.contratos.length?'pointer':'default'}" onclick="${data.contratos.length?`togInline(’${uid}-c’,’${uid}-c-arrow’)`:''}"> <span class="resp-row-label"><span class="resp-row-dot" style="background:var(--blue)"></span>Contratos enviados</span> <span style="display:flex;align-items:center;gap:6px"><span class="resp-row-val" style="color:var(--blue)">${data.contratos.length}</span>${data.contratos.length?`<span id="${uid}-c-arrow" style="font-size:10px;color:var(--muted);display:inline-block;transition:transform .2s">▶</span>`:''}</span> </div>`;
...
h+=`<div class="resp-row" style="cursor:${data.vendas.length?'pointer':'default'}" onclick="${data.vendas.length?`togInline(’${uid}-v’,’${uid}-v-arrow’)`:''}"> <span class="resp-row-label"><span class="resp-row-dot" style="background:var(--green)"></span>Vendas</span> <span style="display:flex;align-items:center;gap:6px"><span class="resp-row-val" style="color:var(--green)">${data.vendas.length}</span>${data.vendas.length?`<span id="${uid}-v-arrow" style="font-size:10px;color:var(--muted);display:inline-block;transition:transform .2s">▶</span>`:''}</span> </div>`;
