...
function renderPane(key){
  ...
  s.vendas.forEach(function(d){const u=uname(d);if(!umap[u])umap[u]={ativo:0,et:{},vendas:[],contratos:[],perdas:0,valor:0};umap[u].vendas.push(d);umap[u].valor+=(d.amount_total||0);});
  ...
  users.forEach(function(entry,i){
    ...
    if(data.contratos.length){h+='<div class="resp-deal-list">';data.contratos.forEach(function(d){h+='<div class="resp-deal-item"><span class="resp-deal-name" title="'+(d.name||'')+'">'+( d.name||'--')+'</span><span class="resp-deal-date">'+(d.data_contrato_fmt||'--')+'</span></div>';});h+='</div>';}
    ...
    if(data.vendas.length){h+='<div class="resp-deal-list">';data.vendas.forEach(function(d){h+='<div class="resp-deal-item"><span class="resp-deal-name" title="'+(d.name||'')+'">'+( d.name||'--')+'</span><span class="resp-deal-date">'+fdate(d.closed_at)+'</span></div>';});h+='</div>';}
  ...
  if(totP){
    ...
    s.perdas.forEach(function(d){const m=(d.deal_lost_reason&&d.deal_lost_reason.name)||'--';h+='<tr><td class="dn">'+( d.name||'--')+'</td><td class="du">'+uname(d)+'</td><td class="dd">'+((d.deal_stage&&d.deal_stage.name)||'--')+'</td><td><span class="pill">'+m+'</span></td></tr>';});
    h+='</tbody></table></div></div>';
  }
  return h;
}
...
function renderTotal(){
  ...
  h+='<div class="motivos-grid">'+msortedT.map(function(e){return '<div class="mc"><span class="mc-n">'+e[0]+'</span><span class="mc-v">'+e[1]+'</span></div>';}).join('')+'</div>';
  if(totP){
...
function renderD1(rpD1,rrrD1,rpHoje,rrrHoje,paneKey){
  ...
  else{d1.forEach(function(d){const stg=d.current_stage?' <span style="font-size:9px;color:var(--teal)">['+d.current_stage+']</span>':'';h+='<div class="d1-item"><div><div class="d1-name">'+( d.name||'--')+stg+'</div><div class="d1-user">'+( d.user||'--')+(paneKey==='total'?' &middot; '+d._f:'')+' &middot; contrato: '+(d.data_contrato_fmt||'--')+'</div></div><span class="d1-tag" style="color:var(--amber);background:var(--amber-dim)">aguard. assin.</span></div>';});}
  ...
  else{dHoje.forEach(function(d){const stg=d.current_stage?' <span style="font-size:9px;color:var(--teal)">['+d.current_stage+']</span>':'';h+='<div class="d1-item"><div><div class="d1-name">'+( d.name||'--')+stg+'</div><div class="d1-user">'+( d.user||'--')+(paneKey==='total'?' &middot; '+d._f:'')+' &middot; contrato: '+(d.data_contrato_fmt||'--')+'</div></div><span class="d1-tag" style="color:var(--blue);background:var(--blue-dim)">hoje</span></div>';});}
  h+='</div></div>';
  return h;
}
...
function renderContratosPorDia(contratos,paneKey){
  ...
  (contratos||[]).forEach(function(d){
    ...
    if(parts[1]!==selM||parts[0]!==selY)return;
